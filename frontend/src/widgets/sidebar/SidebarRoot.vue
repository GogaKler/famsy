...
<template>
  <div>
    ...
    <aside 
      class="fixed top-0 left-0 w-[280px] h-full bg-surface-secondary shadow-xl z-50 transition-transform duration-300 ease-in-out transform"
      :class="{ 'translate-x-0': isOpen, '-translate-x-full': !isOpen }"
    >
      <div class="p-4 flex items-center justify-between border-b border-gray-200">
        <div class="flex items-center gap-2 text-lg font-semibold text-primary">
          <FontAwesomeIcon icon="wallet" class="bg-primary text-white p-1 rounded" />
